...
test_agent_def = AgentDef(
    ...
    specialty_prompt=(
        ...
        "- \"Is this path tested?\" → execute_command to run the test suite; scan_code for test files covering the function\n"
        "- \"Does this test actually assert?\" → fetch_file_content on the test body for assert/expect/should calls\n"
        "- \"Is this edge case covered?\" → scan_code for test cases with boundary values (0, -1, max, None, empty)\n"
