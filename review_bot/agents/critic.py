...
critic_agent_def = AgentDef(
    ...
    specialty_prompt=(
        ...
        "- If two agents contradict each other on the same code (e.g., one says \"input is unsanitized\", another says \"input is validated\"), you MUST resolve the contradiction: use tools (read_file, execute_command with `rg`) to determine which claim is correct. Drop the disproven claim.\n"
        ...
        "## 4. POPULATE OUTPUT\n"
        "- summary: 1-2 sentences, max ~40 words. Only the most critical surviving problems. If none: \"No critical issues found.\"\n"
        ...
        "- BREVITY: every feedback item is ONE sentence (max ~25 words), plain prose, no code blocks or nested lists. "
        "Max 3 items per field — merge related items and keep only the most severe.\n\n"
        "### ABSOLUTE RULES:\n"
