...
`/workspace` is mounted with Podman's `:O` copy-on-write overlay, so agent writes (installs, generated files) never reach the host and do not survive a container restart. If the container dies mid-review, the next tool call restarts it once; if the restart fails, tools return an explicit "sandbox unavailable" error so agents mark findings as not tool-verified.
...
| `GITLAB_URL` | Base URL of the GitLab instance (**required** for webhook server); reviews are pinned to this host |
...
- **Sequential sub-agent execution** (not parallel) — all agents share the same system prompt, maximizing LLM prefix cache hits. Only the specialty suffix differs per agent. The context agent runs first on the full prompt; its transcript is then reused as `message_history` for the five judgment agents, so the MR content and fact-finding exploration are sent to the model once. Judgment agents still reach conclusions independently — only fact-finding is shared; the critic runs fresh on the consolidated reports. Sub-agent prompts enforce a hard tool budget (20 calls), sparse todo bookkeeping (≤3 `update_todo` calls), and batched independent tool calls to bound review time.
- **Webhook host pinning** — the review target is derived from the pinned `GITLAB_URL` env var, never from the payload's `attributes.url`. Events whose MR URL origin (scheme + host + port, case-insensitive) does not match are dropped before submission: a review sends `GITLAB_API_TOKEN` (API headers and the git credential helper) to whichever host the URL names, so knowing the webhook secret alone must not redirect the bot to an attacker-controlled server. The server refuses to start when `GITLAB_URL` is unset, same as for `GITLAB_WEBHOOK_TOKEN`.
...
- **Podman sandbox** — agents execute commands in an isolated container. `ContainerManager` is the single delegation point for all sandbox access. `/workspace` is always mounted with Podman's `:O` copy-on-write overlay, uniformly across backends: agent writes never reach the host — critical for the Git backend, which mounts the user's live working tree — at the accepted cost that dependency installs do not survive the automatic once-per-review container restart (the container stays root either way). Podman caveat: the overlay's lower dir (the host checkout) should not be modified while mounted, so host edits during a long `--backend git` review can make sandbox reads flaky. `systemd-journald` runs inside the container via an image-baked systemd drop-in (`review-container/Dockerfile`) that resets journald's capability bounding set, and journal output is forwarded to the console (`-t` TTY + `ForwardToConsole`) so it is visible via `podman logs`. Third-party tools baked into the sandbox image are installed from version-pinned GitHub release assets with SHA-256 digests hardcoded in the Dockerfile — never `curl | sh` from a moving branch — since that image is the security boundary for agent-executed code.
...
- **Ephemeral RAG** — ChromaDB index is built in-memory per review; no persistent storage. The collection uses `hnsw:space: cosine` so the `1 - distance` reported by `semantic_code_search` is true cosine similarity in [-1, 1], not an unbounded L2 artifact.
- **Dotenv loaded before `review_bot` imports** — both entry points (`cli.py`, `gitlab_webhook.py`) call `load_dotenv()` at module top, *before* importing `review_bot`, because config constants (`AGENT_REQUEST_LIMIT`, `CONFIDENCE_THRESHOLD`, `MAX_LINES`) and the webhook's `_load_config()` are captured at import time.
